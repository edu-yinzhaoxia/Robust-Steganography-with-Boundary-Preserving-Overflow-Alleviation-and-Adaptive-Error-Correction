...
if d==1     %grayscale image
    C_SPATIAL = double(cover_img);
elseif d==3     %color image
    cover_img=rgb2ycbcr(cover_img); %convert RGB to YCbCr
%     cover_img=myrgb2ycbcr(cover_img); %convert RGB to YCbCr
    cover_img_Y=cover_img(:,:,1); %Y
    C_SPATIAL = double(cover_img_Y);
else
    error('error in J_UNIWARD_Asy_cost imread');
end

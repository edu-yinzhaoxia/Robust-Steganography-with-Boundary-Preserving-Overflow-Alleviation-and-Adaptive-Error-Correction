function [stc_n_msg_bit] = stc3Embed(getcover_dir,afterchannel_stego_Path,cover_Path,stego_Path,rho_p,rho_m,rs_encoded_msg)
% 嵌入消息，生成载密图像
% 载体
...
%% 将y转成rgb的隐写图像
getcover_Path = getcover_dir;
rgb=imread(getcover_Path);
ycbcr=rgb2ycbcr(rgb);
y=imread(stego_Path);
ycbcr(:,:,1)=y;
stegos=ycbcr2rgb(ycbcr);
imwrite(stegos,afterchannel_stego_Path,'quality',95);
%%
